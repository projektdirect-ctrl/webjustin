...
export default function AdminHero({ texts, updateText }: Props) {
  return (
          ...
          <div>
            <div className="inline-flex items-center space-x-2 bg-amber-400/10 border border-amber-400/30 px-4 py-2 mb-6 md:mb-8">
              ...
              <span className="text-amber-400 font-bold uppercase tracking-widest text-xs">
                <EditableText
                  value={D(texts, 'hero_badge', 'Ústecký kraj & celá ČR')}
                  onChange={v => updateText('hero_badge', v)}
                  className="text-amber-400 font-bold uppercase tracking-widest text-xs"
                />
              </span>
            </div>
            ...
            <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black text-white uppercase leading-none mb-6 md:mb-8">
              <EditableText
                value={D(texts, 'hero_title_line1', 'Stavařina')}
                onChange={v => updateText('hero_title_line1', v)}
                className="text-white font-black uppercase"
              />
              <br />
              <EditableText
                value={D(texts, 'hero_title_line2', 'pořádně.')}
                onChange={v => updateText('hero_title_line2', v)}
                className="text-blue-300 font-black uppercase"
              />
            </h1>

            <p className="text-white/70 text-base sm:text-lg md:text-xl leading-relaxed mb-8 md:mb-10 max-w-xl">
              <EditableText
                value={D(texts, 'hero_subtitle', 'Kompletní rekonstrukce domů, obklady, zámková dlažba, sádrokartony a zednické práce všeho druhu. Práci děláme poctivě a na čas.')}
                onChange={v => updateText('hero_subtitle', v)}
                multiline
                as="span"
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-10 md:mb-12">
              <span className="bg-amber-400 text-blue-950 px-7 py-4 font-black uppercase tracking-widest text-sm text-center">
                <EditableText
                  value={D(texts, 'hero_cta_primary', 'Chci nacenit projekt')}
                  onChange={v => updateText('hero_cta_primary', v)}
                  className="text-blue-950 font-black uppercase tracking-widest text-sm"
                />
              </span>
              <span className="border-2 border-white/30 text-white px-7 py-4 font-bold uppercase tracking-wider text-sm text-center">
                <EditableText
                  value={D(texts, 'hero_cta_secondary', 'Co umíme')}
                  onChange={v => updateText('hero_cta_secondary', v)}
            ...
            <div className="grid grid-cols-3 gap-3 sm:gap-6">
              <div className="text-center">
                <div className="text-2xl sm:text-3xl font-black text-amber-400">
                  <EditableText value={D(texts, 'hero_stat1_num', '10+')} onChange={v => updateText('hero_stat1_num', v)} className="text-amber-400 font-black" />
                </div>
                <div className="text-white/50 text-xs uppercase tracking-wider mt-1">
                  <EditableText value={D(texts, 'hero_stat1_label', 'Let zkušeností')} onChange={v => updateText('hero_stat1_label', v)} className="text-white/50 text-xs uppercase tracking-wider" />
                </div>
              </div>
              <div className="text-center border-x border-white/10">
                <div className="text-2xl sm:text-3xl font-black text-amber-400">
                  <EditableText value={D(texts, 'hero_stat2_num', '200+')} onChange={v => updateText('hero_stat2_num', v)} className="text-amber-400 font-black" />
                </div>
                <div className="text-white/50 text-xs uppercase tracking-wider mt-1">
                  <EditableText value={D(texts, 'hero_stat2_label', 'Dokončených projektů')} onChange={v => updateText('hero_stat2_label', v)} className="text-white/50 text-xs uppercase tracking-wider" />
                </div>
              </div>
              <div className="text-center">
                <div className="text-2xl sm:text-3xl font-black text-amber-400">
                  <EditableText value={D(texts, 'hero_stat3_num', '100%')} onChange={v => updateText('hero_stat3_num', v)} className="text-amber-400 font-black" />
                </div>
                <div className="text-white/50 text-xs uppercase tracking-wider mt-1">
                  <EditableText value={D(texts, 'hero_stat3_label', 'Spokojených klientů')} onChange={v => updateText('hero_stat3_label', v)} className="text-white/50 text-xs uppercase tracking-wider" />
                </div>
              </div>
            </div>
          </div>
